...
c     ..
     ...
     &    CALL ERRMSG('PLKAVG--temperature or wavenums. wrong',.TRUE.)
      ...
      IF (WNUMLO.eq.WNUMHI) then
          
           PLKAVG = 1.191062D-8 * (WNUMLO*WNUMLO*WNUMLO)  / 
     &      ( exp(C2*WNUMLO / T) - 1.0)
      RETURN
      END IF
      !IF (WNUMLO.ne.0.0.or.WNUMHI.ne.50000.0) then          
!          PLKAVG = SIGDPI * T**4 * CONC * 
!     &     PLKF(V(1)) * C2*(WNUMHI-WNUMLO) / T
!          PLKAVG = 0.5000000000 * SIGDPI * T**4 * CONC * 
!     &     (PLKF(V(1)) + PLKF(V(2)))* C2*(WNUMHI-WNUMLO) / T          
      !RETURN
      !END IF
